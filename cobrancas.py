...
while True:
  ...
  # Tenta localizar o botão "Próxima"
  try:
      botao_proxima = driver.find_element(By.CSS_SELECTOR, 'a.paginate_button.next')
      
      # Scroll até o botão "Próxima" para garantir que ele esteja visível
      ActionChains(driver).move_to_element(botao_proxima).perform()
    
      # Verifica se o botão está habilitado
      if "disabled" in botao_proxima.get_attribute("class"):
          print("Botão 'Próxima' está desabilitado. Todas as páginas foram processadas.")
          break
      else:
          # Clica no botão "Próxima" para ir para a próxima página
          botao_proxima.click()
          time.sleep(2)  # Aguarde o carregamento da próxima página
  except NoSuchElementException:
      print("Botão 'Próxima' não encontrado. Todas as páginas foram processadas.")
      break
